  ...
  computed: {
    isFormValid() {
      ...
      const consentChecked =
        (!this.config.firstModal.consentCheckbox &&
          !this.secondModal.consentCheckbox) ||
        this.consentChecked
...
<template>
  <div v-if="config">
    ...
    <transition name="modal-transition">
      ...
      >
        <div
          class="after-modal max-w-sm absolute text-start rounded-xl pb-2 px-5"
        >
          ...
          <form @submit.prevent="submitForm">
            ...
            <div class="flex items-center flex justify-between">
              <p class="text-violet-300">
                {{ config.formsLayout.titleGender }}
              </p>
              <GenderSelect
                :options="config.formsLayout.gender"
                :selectedGender.sync="selectedGender"
                class="cursor-pointer"
              />
            </div>
            <CheckInput
              v-if="config.firstModal.consentCheckbox"
              inputId="consentCheckbox"
              :label="config.firstModal.shareData"
              :checked="checkedShare"
            />
            <CheckInput
              v-if="config.firstModal.consentCheckbox"
              inputId="consentCheckbox"
              :label="config.firstModal.acceptTerms"
    ...
    <transition name="modal-transition">
      <div
        v-if="showFormModal && isModalOpen('form')"
        class="left-0 top-0 w-screen h-screen absolute text-center flex justify-center items-center z-50 bg-black bg-opacity-50"
      >
        <div
          class="after-modal max-w-sm absolute text-start rounded-xl pb-2 px-5"
    ...
    <transition name="modal-transition">
      <div
        v-if="showSecondFormModal && isModalOpen('news')"
        class="left-0 top-0 w-screen h-screen absolute text-center flex justify-center items-center z-50 bg-black bg-opacity-50"
      >
        <div
          class="video-modal text-white px-5 max-w-sm absolute text-start rounded-xl pb-2"
        >
          ...
          <form @submit.prevent="submitForm">
            ...
            <CheckInput
              v-if="config.secondModal.consentCheckbox"
              inputId="consentCheckbox"
              :label="config.secondModal.shareData"
              :checked="checkedShare"
            />
            <CheckInput
              v-if="config.secondModal.consentCheckbox"
              inputId="consentCheckbox"
              :label="config.secondModal.acceptTerms"
              :checked="consentChecked"
              @update:checked="consentChecked = $event"
            />
            <ButtonSend :is-form-valid="isFormValid" button-text="Enviar" />
